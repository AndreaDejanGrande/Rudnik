...
class BitcoinRPC(object):
    ...
    @defer.inlineCallbacks
    def submitblock(self, block_hex, hash_hex, vcrypt_hex):
  #try 5 times? 500 Internal Server Error could mean random error or that TX messages setting is wrong
        ...
        if json.loads(resp)['result'] == None:
            ...
            defer.returnValue((yield self.blockexists(hash_hex, vcrypt_hex)))
        else:
    ...
    @defer.inlineCallbacks
    def blockexists(self, hash_hex, vcrypt_hex):
        valid_hash = None
        blockheight = None
        # try both hash_hex and vcrypt_hex to find block
        try:
            ...
            try:
                resp = (yield self._call('getblock', [vcrypt_hex,]))
                result = json.loads(resp)['result']
                if "hash" in result and result['hash'] == vcrypt_hex:
                    valid_hash = vcrypt_hex
                    log.debug("Block found: %s" % vcrypt_hex)
                    if "height" in result:
                        blockheight = result['height']
                    else:
                        defer.returnValue(True)
                else:
                    log.info("Cannot find block for %s" % vcrypt_hex)
                    defer.returnValue(False)

            except Exception as e:
                log.info("Cannot find block for hash_hex %s or vcrypt_hex %s" % hash_hex, vcrypt_hex)
                defer.returnValue(False)
