...
import StringIO
import settings
if settings.COINDAEMON_ALGO == 'vcrypt':
    import vdn_vcrypt
elif settings.COINDAEMON_ALGO  == 'vcrypt-jane':
    import yac_vcrypt
...
class TemplateRegistry(object):
    ...
    def submit_share(self, job_id, worker_name, session, extranonce1_bin, extranonce2, ntime, nonce,
        ...
        if settings.COINDAEMON_ALGO == 'vcrypt':
            print('Getting VDN PoW')
            hash_bin = vdn_vcrypt.getPoWHash(''.join([ header_bin[i*4:i*4+4][::-1] for i in range(0, 20) ]))
        elif settings.COINDAEMON_ALGO  == 'vcrypt-jane':
            hash_bin = yac_vcrypt.getPoWHash(''.join([ header_bin[i*4:i*4+4][::-1] for i in range(0, 20) ]), int(ntime, 16))
        elif settings.COINDAEMON_ALGO == 'quark':
        ...
        if settings.COINDAEMON_ALGO == 'vcrypt' or settings.COINDAEMON_ALGO == 'vcrypt-jane':
        ...
        if isBlockCandidate == True:
            ...
            log.info("We found a block candidate! %s" % vcrypt_hash_hex)

            # Reverse the header and get the potential block hash (for vcrypt only) 
            if settings.COINDAEMON_ALGO == 'riecoin':
	    ...
	    on_submit = self.bitcoin_rpc.submitblock(serialized, block_hash_hex, vcrypt_hash_hex)
            if on_submit:
                self.update_block()
            ...
            if settings.SOLUTION_BLOCK_HASH:
                ...
                return (header_hex, vcrypt_hash_hex, share_diff, on_submit)
        
        if settings.SOLUTION_BLOCK_HASH:
        # Reverse the header and get the potential block hash (for vcrypt only) only do this if we want to send in the block hash to the shares table
            if settings.COINDAEMON_ALGO == 'riecoin':
                block_hash_bin = util.doublesha(''.join([ header_bin[i*4:i*4+4][::-1] for i in range(0, 28) ]))
            else:
	        block_hash_bin = util.doublesha(''.join([ header_bin[i*4:i*4+4][::-1] for i in range(0, 20) ]))
            ...
            return (header_hex, vcrypt_hash_hex, share_diff, None)
