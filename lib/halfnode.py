...
class CBlock(object):
    ...
    if settings.COINDAEMON_ALGO == 'vcrypt':
       def calc_vcrypt(self):
           if self.vcrypt is None:
               r = []
               r.append(struct.pack("<i", self.nVersion))
               r.append(ser_uint256(self.hashPrevBlock))
               r.append(ser_uint256(self.hashMerkleRoot))
               ...
               r.append(struct.pack("<I", self.nBits))
               r.append(struct.pack("<I", self.nNonce))
               self.vcrypt = uint256_from_str(vdn_vcrypt.getPoWHash(''.join(r)))
           return self.vcrypt
    elif settings.COINDAEMON_ALGO == 'quark':
         def calc_quark(self):
    ...
    def is_valid(self):
        if settings.COINDAEMON_ALGO == 'riecoin':
            self.calc_riecoin()
        elif settings.COINDAEMON_ALGO == 'vcrypt':
            self.calc_vcrypt()
        elif settings.COINDAEMON_ALGO == 'quark':
            self.calc_quark()
        else:
