...
from coinbasetx import CoinbaseDonationsTransaction
import lib.logger
...
class BlockTemplate(halfnode.CBlock):
    ...
    def fill_from_rpc(self, data):
        ...
        txhashes = [None, None] + [ util.ser_uint256(int(t['hash'], 16)) for t in data['transactions'] ]
        ...
        if settings.COINDAEMON_Reward == 'POW':
            ...
            donations_coinbase = CoinbaseDonationsTransaction(self.timestamper, self.coinbaser, data['coinbasevalue'],
                                              data['coinbaseaux']['flags'], data['height'],
                                              settings.COINBASE_EXTRAS)
        else:
        ...
        self.nTime = 0
        self.nNonce = 0
        self.vtx = [ coinbase, donations_coinbase, ]
        ...
        # print(len(util.doublesha(self.vtx[1]._serialized[0] + self.vtx[1]._serialized[1])))
        
        #self.merkletree._steps.insert(0, util.doublesha(self.vtx[1]._serialized[0] + self.vtx[1]._serialized[1]))
    ...
    def build_broadcast_args(self):
        ...
        (doncoinb1, doncoinb2) = [ binascii.hexlify(x) for x in self.vtx[1]._serialized ]
        ...
        return (job_id, prevhash, coinb1, coinb2, doncoinb1, doncoinb2, merkle_branch, version, nbits, ntime, clean_jobs)
    ...
    def serialize_donations_coinbase(self, extranonce1, extranonce2):
        '''Serialize coinbase with given extranonce1 and extranonce2
        in binary form'''
        (part1, part2) = self.vtx[1]._serialized
        return part1 + extranonce1 + extranonce2 + part2
    ...
    def finalize(self, merkle_root_int, extranonce1_bin, extranonce2_bin, ntime, nonce):
        ...
        self.vtx[1].set_extranonce(extranonce1_bin + extranonce2_bin)        
